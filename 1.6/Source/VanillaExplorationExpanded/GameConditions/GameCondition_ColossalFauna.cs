...
{
    public class GameCondition_ColossalFauna : GameCondition
    {
            ...
            if (tickCounter-- < 0)
            {
                ...
                foreach (Map map in base.AffectedMaps)
                {
                    ...
                    for (int i = 0; i < pawns.Count; i++)
                    {
                        if (pawns[i].IsAnimal)
                        {
                            Hediff hediff = pawns[i].health.hediffSet.GetFirstHediffOfDef(InternalDefOf.VEE_ColossalFauna);
                            if (hediff is null)
                            {
                                pawns[i].health.AddHediff(InternalDefOf.VEE_ColossalFauna);
                            }
