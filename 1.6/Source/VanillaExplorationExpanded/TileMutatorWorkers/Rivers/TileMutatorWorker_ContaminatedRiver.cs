...
{
    public class TileMutatorWorker_ContaminatedRiver : TileMutatorWorker_River
    {


        public TileMutatorWorker_ContaminatedRiver(TileMutatorDef def)
            : base(def)
        {
        }
        ...
        public override void GeneratePostTerrain(Map map)
        {
            ...
            foreach (IntVec3 cell in map.AllCells)
            {
                ...
                {
                    map.terrainGrid.SetTerrain(cell, InternalDefOf.VEE_IrradiatedWaterDeepMovingChestDeep);
                }
                if (cell.GetTerrain(map)== MapGenUtility.ShallowMovingWaterTerrainAt(cell, map))
                {
                    map.terrainGrid.SetTerrain(cell, InternalDefOf.VEE_IrradiatedWaterMovingShallow);
                }
