...
{
    public class TileMutatorWorker_Predators : TileMutatorWorker
    {
        ...
        public TileMutatorWorker_Predators(TileMutatorDef def)
            : base(def)
        {
        }
        ...
        public List<PawnKindDef> GetAnimalKind(PlanetTile tile)
        {
            if (tmpAnimals.Empty()) {
                ...
                if (tmpAnimals.Empty())
                {
                    Log.ErrorOnce($"TileMutatorWorker_Predators: Could not find animal in biome with commonality > {MinAnimalCommonality}, either increase some commonalities, or decrease animalDensity in the biomeDef below this mutators min of {def.animalDensityRange.min}", 123498);
                    return null;
                }
    ...
    public class TileMutatorWorker_ReducedPredators : TileMutatorWorker_Predators
    {
        public override float AnimalCommonalityFactor => 0.25f;
     

        public TileMutatorWorker_ReducedPredators(TileMutatorDef def)
            : base(def)
        {
        }
